...
import TakeOne exposing (rows)
import TakeOne exposing (Cell(..))
...
toCenter (rows,cols) = (floor (toFloat rows / 2), floor (toFloat cols / 2))
...
createCellAndNeighbors (r,c) (a, b) state cells =
   List.foldr (\(a2,b2) acc ->
            if a2 < r && b2 < c && a2 >= 0 && b2 >= 0 
              then  createCellAndNeighbors (r,c) (a2,b2) state acc 
              else acc
              )
...
listToIndexedList cols cells  = 
  List.indexedMap (\ n state -> 
      let row = n // cols
          col = modBy cols n
          coords = (row, col) in 
    (coords, Cell coords state)) cells
...
getCell c grid = Dict.get c grid.cells

getNeighbors : CellCoords -> Grid -> List (Maybe Cell)
getNeighbors coords grid = List.map (\c -> getCell c grid) (findNeighboringCoords coords)

isAlive : Maybe Cell -> Bool
isAlive cell = if Maybe.map (.state) cell == Just Alive then True else False

aliveNeighbors : CellCoords -> Grid -> Int
aliveNeighbors coords grid = List.length (List.filter (isAlive) (getNeighbors coords grid))

willBeAlive : CellCoords -> Grid -> Bool
willBeAlive c grid = 
  if isAlive (getCell c grid) 
    then
        aliveNeighbors c grid == 2 ||
...
updateAlive loc grid = 
    if  aliveNeighbors loc grid == 2 ||
        aliveNeighbors loc grid == 3
        then Cell loc Alive
        else Cell loc Dead

updateDead : CellCoords -> Grid -> Cell
updateDead loc grid = 
    if aliveNeighbors loc grid == 3
    then Cell loc Alive
    else Cell loc Dead

updateCell : Cell -> CellCoords -> Grid -> Cell
updateCell cell loc grid = 
  if cell.state == Alive then updateAlive loc grid else 
    updateDead loc grid
...
showRow n row = tr [] (List.map (\c -> showCell n c) row)

showGrid n grid = List.map (showRow n) (toRows grid)
