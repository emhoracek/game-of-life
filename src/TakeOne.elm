...
columns grid =
    Array.length (Maybe.withDefault Array.empty (get 0 grid))
...
getCell r c grid =
    Maybe.andThen (get c) (get r grid)


getNeighbors : Int -> Int -> Grid -> Array (Maybe Cell)
getNeighbors r c grid =
    fromList
        [ getCell (r - 1) c grid
...
aliveNeighbors r c grid =
    Array.length (Array.filter isAlive (getNeighbors r c grid))


willBeAlive r c grid =
    if isAlive (getCell r c grid) then
        aliveNeighbors r c grid
            == 2
            || aliveNeighbors r c grid
            == 3
...
showRow n row =
    tr [] (toList (Array.map (\c -> showCell n c) row))
